...
def claimCode(token, promoId):
    ...
    try:
        ...
        if proxy:
            response = requests.post('https://discord.com/api/v9/outbound-promotions/' + promoId + '/claim', cookies=cookies, headers=headers, proxies={"http": "http://" + proxyUrl, "https": "http://" + proxyUrl})
        else:
...
def getCode(token):
    ...
    try:
        ...
        print(Fore.CYAN + "[*] Getting Inventory Code [" + token[:20] + "]")
        if proxy:
            response = requests.get('https://discord.com/api/v9/users/@me/outbound-promotions/codes?locale=en-US', cookies=cookies, headers=headers, proxies={"http": "http://" + proxyUrl, "https": "http://" + proxyUrl})
        else:
            response = requests.get('https://discord.com/api/v9/users/@me/outbound-promotions/codes?locale=en-US', cookies=cookies, headers=headers)
        if response.status_code == 200:
            for gift in response.json():
                alreadyCheckedId.append(gift['promotion']['id'])
                print(Fore.GREEN + "[+] Code: " + gift['code'] + "    |   Product: " + gift['promotion']['outbound_title'] + "    |   Id: " + gift['promotion']['id'])
                f = open("code.txt", "a")
                f.write("[+] Code: " + gift['code'] + "    |   Product: " + gift['promotion']['outbound_title'] + "    |   Id: " + gift['promotion']['id'] + "\n")
                f.close()
        else:
            print(Fore.RED + "[!] Error    |   Token: " + token[:20] + "    |   StatusCode: " + str(response.status_code))
        for promoId in promoIds:
            if promoId not in alreadyCheckedId:
                try:
                    claimCode(token, promoId)
                except:
                    claimCode(token, promoId)
...
empt = input("Email Pass Token? [t/f]> ")
if empt == "t":
    for token in tokens:
        token = token.split(":")[2]
        print(Fore.CYAN + "[*] Using Token [" + token[:20] + "]")
        getCode(token)
